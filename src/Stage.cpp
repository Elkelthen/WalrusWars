...
void Stage::generateMap(){
	...
	int j;
	long r;
	for(i = 0; i < ICE_BLOCKS_WIDTH+2; i++){
		for(j = 0; j < ICE_BLOCKS_HEIGHT+2; j++){
...
void Stage::tickMelt(int stage) {
	// point to correct stage to melt
    std::array<std::array<float, ICE_BLOCKS_HEIGHT+2>, ICE_BLOCKS_WIDTH+2>* melting_stage;
	switch (stage) {
		case -2:
	...
	for(int i = 0; i < ICE_BLOCKS_WIDTH+2; i++) {
		for(int j = 0; j < ICE_BLOCKS_HEIGHT+2; j++) {
			if ((*melting_stage)[i][j] > 0) {
				r = rand() % 100 + 1;
				// min accounts for neighbors outside of array
				sum = std::min((*melting_stage)[i-1][j], 1.0f) + std::min((*melting_stage)[i+1][j], 1.0f) + std::min((*melting_stage)[i-1][j-1], 1.0f) + std::min((*melting_stage)[i-1][j+1], 1.0f) + std::min((*melting_stage)[i+1][j-1], 1.0f) + std::min((*melting_stage)[i+1][j+1], 1.0f) + std::min((*melting_stage)[i][j+1], 1.0f) + std::min((*melting_stage)[i][j-1], 1.0f);
				if(sum>=6){
